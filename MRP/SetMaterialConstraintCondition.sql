...
--New query to check parts with LT <=14 to non-constrained material as well
WITH LeadTimeCalc AS ( --Get lead times for all active parts
	SELECT PP.PartNum, PP.Plant, PP.SourceType,
	CASE
		WHEN PP.SourceType = 'P' THEN PP.LeadTime
		WHEN PP.SourceType = 'M' THEN PP.TotMfgLeadTimeSys
		ELSE 10000
	END AS 'LeadTime'
	FROM PartPlant PP
	INNER JOIN Part P ON PP.PartNum = P.PartNum
