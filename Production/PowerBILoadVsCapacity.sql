...
/*
    PowerBILoadVsCapacity.sql

    Description:
    This SQL script generates a dataset for Power BI reporting that compares production load against planned capacity for each resource group and calendar date. 
    It uses recursive CTEs to:
      - Traverse job-product relationships to link jobs to sales orders (ANCHOR).
      - Unpack multi-day load and hour strings into individual records per day (LoadCTE).
      - Retrieve and adjust shop capacity, including user-defined overrides (ShopCapCTE).
      - Build a calendar range based on the earliest and latest relevant dates (bounds, CalendarCTE).
      - Generate a full calendar for each resource group (ResourceCalendar).
      - Join all data to produce a row per resource group per day, with job, load, and capacity details (BASE_2).

    Output:
    Each row represents a resource group and calendar date, with associated job, load, and capacity information, suitable for Power BI analysis.

    Tables Used:
      - erp.JobProd
      - Erp.ResourceTimeUsed
      - Erp.ShopCap
      - Ice.UD04
      - JobHead

    Key Columns in Output:
      - RGResourceGrpID: Resource group ID
      - RGCalendarDate: Calendar date
      - ResourceTimeUsed_JobNum: Job number
      - PartNum: Part number
      - LoadHrs: Load hours for the day
      - Shop_Capacity: Planned shop capacity for the day
      - SOLineRelPartNum: Sales order/line/release or MTS/NA indicator

    Usage:
    Run this script in SQL Server Management Studio to produce a dataset for Power BI load vs. capacity analysis.
*/
WITH ANCHOR  AS (
    SELECT 
        JobNum,
