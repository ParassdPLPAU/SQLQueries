...
WITH ANCHOR  AS (
    ...
    SELECT 
        0 AS 'RN',
        PartNum,
        CAST(GETDATE() AS date) AS 'TPDueDate',
    ...
    SELECT 
        ...
        (COALESCE(PW.NetDailyQty,0) + SUM(DailySums.NetDailyQty) OVER (PARTITION BY DailySums.PartNum ORDER BY DailySums.DueDate))
        AS 'DailyBalance'
    FROM DailySums
